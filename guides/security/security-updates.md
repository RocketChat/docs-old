...
_Details about this security fix will be provided later as per our responsible disclosure policy. (reference: _3t33pd)
{% endhint %}
...
_Details about this security fix will be provided later as per our responsible disclosure policy. (reference: _5nc7fh)
{% endhint %}
...
_Details about this security fix will be provided later as per our responsible disclosure policy. (reference: _2x348d)
{% endhint %}
...
_(**XSS**) An issue with the HTML sanitizer could be exploited, leading to potential Cross-Site-Scripting. (reference: _4bak1j)
{% endhint %}
...
_(**SSRF**) A method could be called without authentication, leading to potential Server-side Request Forgery. (reference: _317v72)
{% endhint %}
...
_(**XSS**) An improperly used front-end library together with a validation bypass in a function could be exploited, leading to potential Cross-Site-Scripting . (reference: _79y67r)
{% endhint %}
...
_(**RCE**) An API endpoint was vulnerable to NoSQL-injection attacks, leading to potential Remote Code Execution. (reference: _3v29b6)
{% endhint %}
...
_(**IDOR**) An IDOR vulnerability could be used to export data of other users. (reference: _6tr8jg)
{% endhint %}
...
_(**NoSQL injection**) An unauthenticated method could be exploited for a potential NoSQL injection, resulting in potential account takeover. (reference: _3v299a)
{% endhint %}
...
_(**NoSQL injection**) An authenticated endpoint could be exploited for a potential NoSQL injection, resulting in potential account takeover. (reference: _3v29b6)
{% endhint %}
...
_(**XSS**) An improper input sanitization in the livechat widget could be exploited for an XSS attack. Variation of 0023. (reference: _3h5cty)
{% endhint %}
...
_**(XSS**) An improper input sanitization in the livechat widget could be exploited for an XSS attack. (reference: _3h5cty)
{% endhint %}
...
_(**Self-XSS) **An issue with SVG files not being sanitized allowed for potential self XSS. (reference: _98bfae)
{% endhint %}
...
_(**Email enumeration) **An issue with an API method allowed for potential email enumeration. (reference: _6tr8gn)
{% endhint %}
...
_(**ReDOS) **An issue with certain regular expressions could lead potentially to Denial of Service. (reference: _2q6wxz)
{% endhint %}
...
_(**Authentication) **An authentication issue allowed adding users to rooms without proper authorization. (reference: _330zt5_)_
{% endhint %}
