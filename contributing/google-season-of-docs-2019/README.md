...
In this project, we will **not** be creating the playground or the docs generation systems, that will be handled by the Google Summer of Code student ([See the GSoC Project]()). Initially some of the endpoints will not be able to be on the playground or generated. The objective of this project is to find a way to add these missing endpoints to the playground system and find ways to improve our api documentation.
...
**Configuration**: We need to complete our settings, fortunately we already have the skeleton for that documentation on [This Pull Request](https://github.com/RocketChat/docs/pull/1095) so we only need to _cherry-pick_ the desired file and complete it.
