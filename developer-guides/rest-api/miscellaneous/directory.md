...
| :--- | :--- | :--- | :--- |
| `query` | `{"text": "rocket", "type": "users", "workspace": "local"}` | Required | When `type` is `users` you can send an additional `workspace` field, that can be `local` \(default\) or `all`. `workspace=all` will work only if [Federation](../../../administrator-guides/federation.md) is enabled. |
