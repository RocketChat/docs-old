...
A **Role** is a set of permissions you can assign to users. Rocket.Chat already comes with a set of default roles that can be edited. You can also create new roles for users.
...
Roles can have different scopes. Currently we have two scopes **Global** and **Rooms**. The permissions of the Global roles are valid for the whole server, while the Rooms roles only apply to individual channels, where a user must be elected to the specific role in the specified channel.

An example of `Global` role would be an `admin` who has valid permissions for the whole server, while a `moderator` with a `Rooms` role would have permissions only in a channel that they were elected for that role.

By default, roles display as little tags on the side of a username on messages.

### Default Roles

- **admin** `(Global)` - has access to all settings and administrator tools.
...
- **guest** `(Global)` - anonymous users that want to write and participate in channels when the `Allow Anonymous Read` and `Allow Anonymous Write` settings are active.
- **leader** `(Rooms)` - no special permissions, but used to set a `leader` to a channel. Leaders appear on the header of a channel.
- **moderator** `(Rooms)` - has moderation permissions for a channel and must be elected by a channel owner.
- **owner** `(Rooms)` - has owner permissions for a channel. When a user creates a channel they become the owner of that channel. They can also elect more owners for that channel.
- **user** `(Global)` - regular user rights. Most users receive this role when they sign in.
- **livechat-agent** `(Global)` - agents of Livechat who can respond to Livechat requests.
- **livechat-manager** `(Global)` - manager of Livechat who can manage agents and guests.
- **livechat-guest** `(Global)` - guest users coming from a Livechat channel.
- **auditor** `(Global)` - Enterprise users only. Users with this role can review messages between users in the Message Auditing Panel.
- **auditor-log** `(Global)` - Enterprise users only. Users with this role can check details in the Auditing Log about the individuals who used the Message Auditing Panel.
- **monitor** `(Global)` - Enterprise users only. Users with this role can monitor the specified department or departments, without receiving permissions to manage the omnichannel as a whole, which makes the role different from the Livechat Manager.
...
| api-bypass-rate-limit | Permission to call an API without rate limitation. See [Rate Limiter](https://rocket.chat/docs/developer-guides/rest-api/rate-limiter/).
| archive-room | Permission to archive a channel.
| assign-admin-role | Permission to promote a user to admin. Requires `view-user-administration` permission. Accessible from **Administration > Users**.
...
| call-management | Permission to start a meeting. Requires **Video Conference** with **BigBlueButton** enabled. Accessible from **More > BBB Video Chat > Start Meeting**.
| clean-channel-history | Permission to purge channel messages and/or files.
| close-livechat-room | Permission to close your own live chat channels.
| close-others-livechat-room | Permission to close other Livechat channels.
| can-audit | Permission for Enterprise users to access to the **Message Auditing Panel**.
| can-audit-log | Permission for Enterprise users to access to the **Message Auditing Log**.
...
| edit-room-retention-policy | Permission to edit the room retention policy.
...
| manage-assets | Permission to manage assets. Must also be an admin. Accessible from **Administration > Assets**.
...
| manage-own-outgoing-integrations | User can create and edit their own outgoing webhooks for integration.
| manage-own-incoming-integrations | User can create and edit their own incoming webhooks for integration.
...
| send-many-messages | Permission to bypasses rate limit of five messages per second.
...
| set-readonly | Permission to set a room to be read-only. Accessible from **Room Info > Edit**.
...
| view-l-room | Permission to view Livechat channel.
| view-livechat-analytics | Permission to view Livechat analytics. Requires Livechat feature enabled and view-livechat-manager permission.
