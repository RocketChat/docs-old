...
There are two types of integrations with GitHub:
...
3. Select the desired account to post the alerts (you should create a dedicated account just for notifications)
4. Set the "Enable Scripts" option to `True`
5. Copy-paste one of the example scripts below
6. Save the integration to generate a webhook URL and secret for you
...
This script generates notifications for the following repository events:
...
The purpose of the integration script is to transform data in one format (the format provided by your incoming service, such as GitHub) into another format (the format expected by Rocket.Chat). Therefore, when you decide to customize either of the scripts presented above, you need two resources:
...
- Remember to **save your integration** before closing
